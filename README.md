...
3.1. (You can follow this guide to generate a Serial Number [iMessage](https://www.tonymacx86.com/threads/an-idiots-guide-to-imessage.196827/) and this site to verify it [EveryMac](http://www.everymac.com/ultimate-mac-lookup/))
3.2. If the system is already recognizing Board Serial, UUID and MAC skip this step -> Open your config.plist and populate the Serial, Board Serial, UUID and MAC address. Make sure to edit the config.plist only with ProperTree.
4. Go to System Preferences > Startup Disk and select your startup disk.
5. Disable CFG Lock and save profile in Bios (see below).
